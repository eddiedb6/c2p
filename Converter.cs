...
namespace c2p
{
    ...
    {
        // Following code and pinyin array is GBK based, and both are with length 396
        private static int[] chCode = new int[] {
        ...
        private static string[] chPinyin = new string[] {
            "a", "ai", "an", "ang", "ao",
            "ba", "bai", "ban", "bang", "bao", "bei", "ben", "beng", "bi", "bian", "biao", "bie", "bin", "bing", "bo", "bu", "ba",
            "cai", "can", "cang", "cao", "ce", "ceng", "cha", "chai", "chan", "chang", "chao", "che", "chen", "cheng", "chi", "chong", "chou", "chu", "chuai", "chuan", "chuang", "chui", "chun", "chuo", "ci", "cong", "cou", "cu", "cuan", "cui", "cun", "cuo",
        ...
        {
            // Check if it's Unicode of Chinese
            ...
            try
            {
                // Convert to GBK code from Unicode
                Encoding gbk = Encoding.GetEncoding(936);
                byte[] arr = gbk.GetBytes(ch.ToString());
                int m1 = (short)(arr[0]);
                int m2 = (short)(arr[1]);
                ...
                if (asc > 0 && asc < 160)
                {
                    System.Console.WriteLine("Is ASCII? " + ch.ToString());
                    return ch.ToString();
                }
                else
                {
                    for (int i = chCode.Length - 1; i >= 0; --i)
                    {
                        if (chCode[i] <= asc)
                        {
                            return chPinyin[i];
                    ...
                    System.Console.WriteLine("Failed to find py for " + ch.ToString());
                }
            }
            catch (Exception e)
            {
                System.Console.WriteLine("Exception in converter");
